...
func initialize(c echo.Context) error {
	...
	//30秒制限に掛かりそうならgoroutineを考える
	if err := loadChairs(); err != nil {
		c.Logger().Errorf("Load chairs error : %v", err)
	}
	if err := loadEstates(); err != nil {
		c.Logger().Errorf("Load estates error : %v", err)
	}
...
func searchChairs(c echo.Context) error {
	...
	err = db.Select(&chairs, searchQuery+searchCondition, params...)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	chairsLen := len(chairs)
	res.Count = int64(chairsLen)
	sort.SliceStable(chairs, func(i, j int) bool {
		if chairs[i].Popularity == chairs[j].Popularity {
			return chairs[i].ID < chairs[j].ID
		}
		return chairs[i].Popularity > chairs[j].Popularity
	})
	if chairsLen <= page*perPage {
		res.Chairs = []Chair{}
	} else if perPage+page*perPage < chairsLen {
		res.Chairs = chairs[page*perPage : page*perPage+perPage]
	} else {
		res.Chairs = chairs[page*perPage : chairsLen]
	}
...
func searchEstates(c echo.Context) error {
	...
	searchQuery := "SELECT id,thumbnail,name,description,latitude,longitude,address,rent,door_height,door_width,features,popularity FROM estate "
	countQuery := "SELECT COUNT(*) FROM estate "
	if c.QueryParam("rentRangeId") != "" {
		searchQuery += "FORCE INDEX(i2) "
		countQuery += "FORCE INDEX(i2) "
	}
	searchQuery += "WHERE "
	countQuery += "WHERE "
...
func getLowPricedEstate(c echo.Context) error {
	...
	threshRent := 35000
	query := `SELECT id,thumbnail,name,description,latitude,longitude,address,rent,door_height,door_width,features,popularity FROM estate WHERE rent < ? ORDER BY rent ASC, id ASC LIMIT ?`
	err := db.Select(&estates, query, threshRent, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
...
func searchEstateNazotte(c echo.Context) error {
	...
	/*
		...
		for _, estate := range estatesInBoundingBox {
			...
			//		query := fmt.Sprintf(`SELECT id,thumbnail,name,description,latitude,longitude,address,rent,door_height,door_width,features,popularity FROM estate WHERE id = ? AND ST_Contains(ST_PolygonFromText(%s), geo)`, coordinates.coordinatesToText())
